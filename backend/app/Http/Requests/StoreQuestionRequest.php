    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($exam && $exam->status === 'closed') {
                $validator->errors()->add('exam_id', 'Cannot add questions to a closed exam');
            }

            // Check if marks exceed exam total marks
            // Only enforce when exam has a defined, positive total_marks
            if ($exam && isset($exam->total_marks) && is_numeric($exam->total_marks) && $exam->total_marks > 0) {
                $marks = is_numeric($this->marks) ? (float) $this->marks : 0;
                if ($marks > (float) $exam->total_marks) {
